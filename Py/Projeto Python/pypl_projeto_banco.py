...
while True:  # evento loop
    ...
    if event == sg.WIN_CLOSED or event == 'Exit' or event == 'Finalizar':
        ...
        sg.popup('Bem vindo ', values['-user-'], image=sg.EMOJI_BASE64_HAPPY_JOY,
                 keep_on_top=True)  # pop-up de boas vindas
    elif event == 'Cadastro':
        banco()
'''        
        nome = values['-nome-']
        ...
        exibir_valores(nome, email, telefone)

        banco_de_dados = BancoDeDados("127.0.0.1", "root", "", "dbpython")
        banco_de_dados.conectar()

        banco_de_dados.inserir_contato(nome, email, telefone)
